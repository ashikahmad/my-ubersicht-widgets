  ...
  @run '''
   ...
   ''', (err, output) ->
      ...
      # geolocation.getCurrentPosition (geo) =>
        # $('.number-unit', el).html " #{geo.position.coords.latitude}, #{geo.position.coords.longitude}"


      # $('.number', el).html "#{dataIn}<span style='color: #{colors.in}'>⬇︎</span> #{dataOut}<span style='color: #{colors.out}'>⬆︎</span> #{units}"
      $('.number-in', el).html "<span style='color: #{colors.in}'>⬇︎</span>#{dataIn}"
      $('.number-out', el).html "<span style='color: #{colors.out}'>⬆︎</span>#{dataOut}"
      $('.number-unit', el).html " #{units}"
